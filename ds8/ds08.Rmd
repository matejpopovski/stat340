...
author: "Names: Jackson Kaiman, Alex Holland, Matej Popovski, Zoe Weinstein"
...
```{r}
quakes <- read.csv('/Users/Jackson/Documents/STAT340/data/quakes.csv') #Changed location to data folder.
...
ggplot(quakes.count, aes(x = mag.binned, y = N, color = factor(year))) +
  geom_point() +
  scale_y_log10() + 
  labs(title = "Log-Transformed Earthquake Counts by Magnitude",
       x = "Magnitude (M)",
       y = "Cumulative Count (log10 scale)",
       color = "Year") +
  theme_minimal()
...
# Fit a linear model to log10(N) ~ mag.binned
model <- lm(log10(N) ~ mag.binned, data = quakes.count)

# View the summary of the model to see coefficient estimates, p-values, and other relevant information
summary(model)
...
# Extract coefficients and adjust sign for b
coefficients <- coef(model)
a <- unname(coefficients[1])  # Intercept term, which corresponds to a
b <- -unname(coefficients[2])  # Slope term, which corresponds to -b in GR law

# Display the estimates for a and b
cat("Estimated a:", a, "\n")
cat("Estimated b:", b, "\n")
cat("Sum of a + b:", a + b, "\n")
...
ggplot(quakes.count, aes(x = mag.binned, y = log10(N))) +
  ...
  geom_abline(intercept = a, slope = -b, color = "blue", linetype = "dashed") +
  labs(title = "Log-Transformed Earthquake Counts by Magnitude with Fitted Line",
       x = "Magnitude (M)",
       y = "log10(Cumulative Count N)",
       color = "Year") +
  theme_minimal()
...
a_conf_int <- conf_intervals[1, ]
b_conf_int <- -conf_intervals[2, ]
cat("95% Confidence Interval for a:", a_conf_int, "\n")
cat("95% Confidence Interval for b:", b_conf_int, "\n")
...
The 95% confidence intervals for  a  and  b  provide a range in which we expect the true values of these parameters to fall 95% of the time if we were to repeat this sampling and modeling process.
For example, if  a ’s confidence interval is [9.2, 10.1] and  b ’s confidence interval is [1.0, 1.5], we are 95% confident that the actual values of a and b lie within these ranges.
...
This value of  N\_\>5  shows the estimated average number of earthquakes per year with a magnitude of 5 or greater.
...
N_9.5 <- 10^log10_N_9.5
N_10 <- 10^log10_N_10

frequency_9.5_to_10 <- N_9.5 - N_10
frequency_9.5_to_10
```
...
```{r}
# TODO: perform calculations here
avg_years_between_9.5_to_10 <- 1 / frequency_9.5_to_10
avg_years_between_9.5_to_10
```
...
```{r}
# TODO: perform calculations here
log10_N5_lower <- a_conf_int[1] - b * 5
log10_N5_upper <- a_conf_int[2] - b * 5

N5_lower <- 10^log10_N5_lower
N5_upper <- 10^log10_N5_upper

cat("95% Confidence Interval for N_≥5: [", N5_lower, ",", N5_upper, "]\n")
...
```{r}
urbanPop = read.csv("/Users/Jackson/Documents/STAT340/data/urbanPop_sample.csv")
...
TODO: Replace this line with your interpretation!
