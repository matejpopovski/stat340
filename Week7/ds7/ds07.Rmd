...
```{r}
# Change the distribution parameters and true statistics
Nrep <- 1000
nvals <- c(5, 10, 40)
lambda <- 5  # Rate parameter for Poisson distribution (mean and variance)

# True mean and variance for the Poisson distribution
truemean <- lambda
truevar <- lambda

# Repeat the remaining code as before
nvec <- rep(nvals, each = Nrep)
reps <- rep(NA, Nrep * length(nvals))
results <- data.frame("n" = as.factor(nvec), "reps" = reps)
...
pp <- ggplot(results, aes(x = reps))
pp <- pp + geom_histogram(aes(), bins = 30)
pp <- pp + facet_wrap(~n)
pp
...
# Step 1: Estimate p from the data
...
# Step 3: Calculate confidence interval using quantiles
...
conf_level <- 0.95
alpha <- 1 - conf_level
ci <- quantile(replicates, probs = c(alpha / 2, 1 - alpha / 2))
...
> the true value p=0.8 lies within the interval ci.
